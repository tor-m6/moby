...
//extern mlock
func c_mlock(buf *byte, count syscall.Size_t) _C_int
func Mlock(p []byte) (err error) {
	var _p2 *byte
	if len(p) > 0 {
		_p2 = (*byte)(unsafe.Pointer(&p[0]))
	} else {
	...
	syscall.Entersyscall()
	_r := c_mlock(_p2, syscall.Size_t(len(p)))
	var errno syscall.Errno
	setErrno := false
	if _r < 0 {
		errno = syscall.GetErrno()
...
//extern munlock
func c_munlock(buf *byte, count syscall.Size_t) _C_int
func Munlock(p []byte) (err error) {
	var _p2 *byte
	if len(p) > 0 {
		_p2 = (*byte)(unsafe.Pointer(&p[0]))
	} else {
	...
	syscall.Entersyscall()
	_r := c_munlock(_p2, syscall.Size_t(len(p)))
	var errno syscall.Errno
	setErrno := false
	if _r < 0 {
		errno = syscall.GetErrno()
...
// Automatically generated wrapper for Unmount/unmount
//go:noescape
//extern unmount
...
func Unmount(dir string, flags int) (err error) {
	...
	syscall.Entersyscall()
	_r := c_unmount(_p1, _C_long(flags))
	var errno syscall.Errno
	setErrno := false
	if _r < 0 {
		errno = syscall.GetErrno()
...
//go:noescape
//extern uname
func c_uname(buf *syscall.Utsname) _C_int
func Uname(buf *syscall.Utsname) (err error) {
	_r := c_uname(buf)
	var errno syscall.Errno
	setErrno := false
	if _r < 0 {
